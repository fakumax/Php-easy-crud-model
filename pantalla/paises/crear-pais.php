...
if (isset($_POST['submit']) || isset($_POST['actualizar'])) {
    ...
    $sql     = "UPDATE {$tabla} SET {$nombre_pais}=? {$codigo_pais}=? WHERE {$nombre_id}=?";
    $stmt    = $db->prepare($sql);
    $ok      = $stmt->execute([$nombre_pais, $codigo_pais,  $id]);
            ...
            <form method="post">
                <div class="pb-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="nombre_pais" id="nombre_pais" class="form-control" value="<?php echo isset($_GET['modificar']) ? $_GET['modificar'] : ""; ?>">
                    </div>
                    <div class="form-group">
                        <label for="codigo_pais">Código pais</label>
                        <input type="text" name="codigo_pais" id="codigo_pais" class="form-control" value="<?php echo isset($_GET['modificar']) ? $_GET['modificar'] : ""; ?>">
                    </div>
                </div>
                <div class="form-group">
